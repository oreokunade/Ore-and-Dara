...
export const AdminModal: FC<AdminDashboardProps> = ({ isOpen, onClose, onNotify, role = 'master' }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (role === 'bridesfamily' || role === 'groomsfamily') {
        setActiveTab('codes');
      }
      ...
      const load = async () => {
        ...
        } else if (role === 'custom1964') {
          // Custom 1964 gets to see RSVPs for guests who used codes generated by them
          const [loadedRsvps, loadedCodes] = await Promise.all([
            getStoredRsvps(),
            getInviteCodes('custom1964')
          ]);
          setCodes(loadedCodes);
          
          // Filter RSVPs to only those whose names match the 'used_by' on 1964's invite codes
    ...
    >
      ...
      <div className="flex items-center gap-2 px-6 sm:px-12 pt-8 pb-0 shrink-0 overflow-x-auto bg-brand-ivory z-10 border-b border-brand-sand/50">
        {(role === 'master' || role === 'custom1964') && (
          ...
          >
            <Users className="w-4 h-4" /> {role === 'custom1964' ? 'My Guests RSVPs' : 'Guest RSVPs'}
          </button>
        )}
        {role === 'master' && (
          <button
